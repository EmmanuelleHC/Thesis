...
def evaluate_transaction(transaction):
    try:
        ...
        logging.info(f"Transaction evaluated: Label={label}, Risk Score={risk_score}")
...
def consume_and_evaluate(window_size=100000, output_folder="Rule_Based", timeout=500000):
    ...
    try:
        for message in consumer:
            ...
            if count % window_size == 0 or (time.time() - last_message_time > timeout and len(results) > 0):
