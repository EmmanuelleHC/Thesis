...
import pandas as pd
import shutil
from gat_lstm_model import EdgeGAT_LSTM, GraphDataProcessor
...
if __name__ == "__main__":
    ...
    checkpoint = torch.load('hybrid_gat_lstm_model.pth', map_location=device)
    model_params = {k: v for k, v in checkpoint['hyperparameters'].items() if k != 'lr'}
    model = EdgeGAT_LSTM(**model_params)
    model.to(device)
    model.eval()

    output_directory = 'Hybrid_GAT_LSTM'
