...
def evaluate_transaction(row, aml_sim):
    transaction_type_map = {'CRYPTO-TRANSFER': 1, 'PAYMENT': 1}
...
class EdgeGCN_LSTM(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        sender_features = x[edge_index[0]].contiguous()
        receiver_features = x[edge_index[1]].contiguous()
        edge_features = torch.cat([sender_features, receiver_features, edge_attr], dim=1).contiguous()

        print(f"edge_features shape: {edge_features.shape}")  # Should be (batch_size, num_edges, input_size)

        edge_features = edge_features.unsqueeze(0).contiguous()
        lstm_out, _ = self.lstm(edge_features)
        lstm_out = lstm_out.squeeze(0).contiguous()
...
class GraphDataProcessor:
    ...
    def prepare_graph_data(self):
        ...
        # edge_attr = torch.cat([transaction_type_encoded, usd_amount], dim=1)
...
val_data = val_processor.prepare_graph_data2()

train_loader = DataLoader([train_data], batch_size=32, shuffle=True)  # Ensure batch size is appropriate
