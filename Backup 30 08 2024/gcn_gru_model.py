...
class EdgeGCN_GRU(nn.Module):
    def __init__(self, hidden_channels, gru_hidden_channels, out_channels, dropout_rate, num_layers, l2_lambda):
        ...
        self.gru = nn.GRU(
            input_size=hidden_channels * 2 + 3,  # input size: hidden dimension of GCN outputs + number of edge attributes
            hidden_size=gru_hidden_channels,
            num_layers=num_layers,
            batch_first=True,
            dropout=dropout_rate if num_layers > 1 else 0  # GRU dropout can only be applied if num_layers > 1
        )
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        edge_features = edge_features.unsqueeze(0)  # Batch dimension
        gru_out, _ = self.gru(edge_features)
        gru_out = gru_out.squeeze(0)  # Remove batch dimension for linear layer
        out = F.relu(self.lin1(gru_out))
        out = self.lin2(out)
        return out.view(-1)  # Flatten output for loss calculation
