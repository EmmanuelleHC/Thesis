...
import pandas as pd
import shutil
from gcn_gru_model import EdgeGCN_GRU, GraphDataProcessor
...
if __name__ == "__main__":
    ...
    checkpoint = torch.load('hybrid_gcn_gru_model.pth', map_location=device)
    model_params = {k: v for k, v in checkpoint['hyperparameters'].items() if k != 'lr'}
    model = EdgeGCN_GRU(**model_params)
    model.to(device)
    model.eval()

    output_directory = 'Hybrid_GCN_GRU'
